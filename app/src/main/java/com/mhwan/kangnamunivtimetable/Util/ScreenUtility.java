...
public class ScreenUtility {
    ...
    public static ScreenUtility getInstance() {
        if (instance == null)
            instance = new ScreenUtility(AppContext.getContext());
        return instance;
    }

    public ScreenUtility(Context ctx) {
    ...
    public int dpToPx(int dp) {
        ...
        int px = Math.round(dp * ((float) AppContext.getContext().getResources().getDisplayMetrics().densityDpi / DisplayMetrics.DENSITY_DEFAULT));
        return px;
    }

    public int pxToDp(int px) {
        int dp = Math.round(px / ((float) AppContext.getContext().getResources().getDisplayMetrics().densityDpi / DisplayMetrics.DENSITY_DEFAULT));
        return dp;
    }
}
